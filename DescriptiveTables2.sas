...
ods _ALL_ close; 
ods rtf FILE='H:\SAS\output\DescriptiveTables_AF.rtf' bodytitle startpage=no KEEPN;	
...
data temp;
set &datasource;
if premdiscflag='P' then premdiscflag2='Premium ($million)';
if premdiscflag='D' then premdiscflag2='Discount($million)';
...
PROC TABULATE DATA=temp
    ...
    CLASS OrigTerm / ORDER=UNFORMATTED MLF;
	CLASS premdiscflag2/ ORDER=UNFORMATTED MLF;
    CLASS WALArank		/  ORDER=UNFORMATTED;
...
PROC TABULATE DATA=&datasource
    ...
    TABLE 
		/* Page Dimensions */
		premdiscflag={LABEL=''} All={LABEL='Net ($/$100)'} ,
		/* Row Dimension */
        WACrank={LABEL=''}*
          premDiscPct={LABEL=''}*
            Mean={LABEL=''} 
